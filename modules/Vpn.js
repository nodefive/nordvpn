...
export default class Vpn {
    ...
    async setAutoconnect(enabled) {
        const cmd = enabled ? CMD_AUTOCONNECT_ON.trim() : CMD_AUTOCONNECT_OFF;
        await this._procCom.execCommunicateAsync(cmd);
    }
    ...
    async getCountriesList(running = true) {
        ...
        const cleaned = countries.map(c => c.replace(/_/g, ' ').trim()).filter(c => c.length > 0);
    ...
    async getCountriesWithCities() {
        try {
            return await this._httpGet(`https://api.nordvpn.com/v1/servers/countries`);
        } catch (e) {
            this._log.warn('getCountriesWithCities failed', {error: e?.message});
            return null;
        }
    }

    async getCitiesForCountry(country, running = true) {
        const countryArg = country.replace(/ /g, '_');
        const standardOut = await this._execAsyncIfRunning(`${CMD_CITIES} ${countryArg}`, running);
        const cities = this._vpnUtils.processCityCountryOutput(standardOut);
        return cities.map(c => c.replace(/_/g, ' ').trim()).filter(c => c.length > 0);
    }

    async getServersForCity(countryId, cityId, limit = 10) {
        let url = `https://api.nordvpn.com/v1/servers/recommendations?limit=${limit}`;
        url += `&filters[country_id]=${countryId}`;
        url += `&filters[city][id]=${cityId}`;
        ...
        if (technology === 'NORDLYNX') {
            ...
            if (protocol === 'UDP') {
                url += obfuscate
                    ? `&filters[servers_technologies][identifier]=openvpn_xor_udp`
                    : `&filters[servers_technologies][identifier]=openvpn_udp`;
            } else if (protocol === 'TCP') {
                url += obfuscate
                    ? `&filters[servers_technologies][identifier]=openvpn_xor_tcp`
        ...
        try {
            ...
            this._log.endTimer(t, 'SPAN', {operation: 'getServersForCity', countryId, cityId, count: data?.length ?? 0});
            if (!data) return [];
            return data.map(s => ({
                name: s.name,
                hostname: s.hostname.replace('.nordvpn.com', '')
            }));
        } catch (e) {
            this._log.endTimer(t, 'SPAN', {operation: 'getServersForCity', error: e?.message});
            return [];
        }
    }
